...
class Audio {
  ...
  update = () => {
    ...
    this.audio.volume = lerp(this.audio.volume, 1, 0.001);
    ...
    if (this.showPreview) {
      ...
      this.ctx.rect(
        0,
        this.canvas.height - MARGIN - H_BEAT_BAR,
        x * (this.beatCutOff / this.lastBeat) * 0.8,
        H_BEAT_BAR
      );
      ...
      this.ctx.rect(
        0,
        this.canvas.height - MARGIN - H_BEAT_BAR,
        x * (this.volume / this.lastBeat) * 0.8,
        H_BEAT_BAR
      );
      ...
      this.ctx.fillText(
        "Volume:" + parseFloat(this.volume).toFixed(2),
        6,
        H_CANVAS + MARGIN * 2
      );
  ...
  detectBeat = (dt) => {
    ...
    if (
      ...
      this.volume > this.beatCutOff &&
      this.volume > this.BEAT_MIN
    ) {
      if (this.debug) {
